...
{
    ret_code_t err_code;
    ble_audio_acc_init_t init = {0};

    init.audio_write_handler = audio_write_handler;

    err_code = ble_audio_acc_init(&m_audio_acc_service, &init);
    APP_ERROR_CHECK(err_code);
}
...
{
    ret_code_t err_code;
    audio_player_init_t init = {0};

    init.playback_complete_handler = audio_playback_complete;

    err_code = audio_player_init(&init);
    APP_ERROR_CHECK(err_code);
}
